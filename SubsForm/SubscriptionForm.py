        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if subscriptions:
                    ...
                    # Labels for headers
                    tk.Label(view_window, text="Client Name").grid(row=0, column=0)
                    tk.Label(view_window, text="Product Name").grid(row=0, column=1)
                    tk.Label(view_window, text="End Date").grid(row=0, column=2)

                    for i, subscription in enumerate(subscriptions, start=1):
                        tk.Label(view_window, text=subscription["client_name"]).grid(row=i, column=0)
                        tk.Label(view_window, text=subscription["product_name"]).grid(row=i, column=1)
                        tk.Label(view_window, text=subscription["end_date"]).grid(row=i, column=2)
                else:
